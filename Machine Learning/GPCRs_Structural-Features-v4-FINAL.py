...
# 
# This section loads the processed dataset containing GPCR plasma membrane expression (PME) scores and a curated set of biologically relevant features. Only specific structural features are used for model training and analysis are extracted here.
# :::
...
selected_features = ['compound_name', 'GPCR_PME', 'sasa_total', 'sasa_crg', 'sasa_plr', 'sasa_aplr', 'f_crg', 'f_plr', 'f_aplr', 'alpha_helix', '3_10_helix', 'extended_configuration', 
...
# Version 4: SMOTE + GridSearchCV hyperparameter tuning
...
from imblearn.pipeline import Pipeline  # ✅ NEW: for leak-free CV
import pandas as pd
import numpy as np
import seaborn as sns
import matplotlib.pyplot as plt
...
np.random.seed(42)
explainer    = shap.TreeExplainer(rf_model.named_steps['rf'])  # ✅ SHAP only on trained RF
...
plt.savefig('shap_protein-structural_features.pdf', format='pdf', dpi=300, bbox_inches='tight')
plt.show()
...
# :::{markdown}
# ## 5. Model comparison evaluated via 5-fold cross-validation to:
# 1. Assess generalization beyond single train-test split
# 2. Mitigate overfitting concerns
# 3. Provide uncertainty estimates for performance metrics
# 
# :::
...
# Use properly scaled training data for fair comparison
X_comparison = X_train_scaled
y_comparison = y_train
...
# Create results DataFrame and sort by recall (most important for PME prediction)
results_df = pd.DataFrame(results).sort_values('Recall', ascending=False)

print(f"\n✅ Cross-Validation Results (5-fold):")
print(results_df.to_string(index=False, float_format='%.3f'))

# Highlight top performers by recall
...
from IPython.display import display, HTML
display(HTML("<b style='color:green'>✅ All cells executed successfully!</b>"))
print("Execution time:", pd.Timestamp.now())
