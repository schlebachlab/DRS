...
def main():
    ...
    psite_counts = defaultdict(Counter)
    skipped = 0
    bam.reset()
    for r in bam.fetch(until_eof=True):
        L = r.query_length
        ...
        psite = r.reference_start + off # 0-based
        ctg = r.reference_name
        if ctg not in cds_coords:
            skipped += 1
            continue
    ...
    rows = []
    for tx, counter in psite_counts.items():
        for psite_nt, cnt in counter.items():
            rows.append((tx, psite_nt, cnt))
    out_psite = pd.DataFrame(rows, columns=["transcript", "psite_nt", "count"])
    out_psite.to_csv(f"{args.out_prefix}_psite_counts.csv", index=False)
